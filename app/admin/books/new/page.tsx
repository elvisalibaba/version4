...
import { AdminBookImportForm } from "@/components/admin/books/admin-book-import-form";
  ...
  return <div className="space-y-5 pb-10">
    ...
    <AdminBookImportForm>
      {Array.from({ length: 5 }, (_, index) => <fieldset key={index} className="rounded-[26px] border border-[#ded3c2] bg-white p-5"><legend className="px-2 font-serif text-xl text-[#17231d]">Livre {index + 1}{index > 0 ? <span className="ml-2 text-xs font-sans font-normal text-[#837a70]">facultatif</span> : null}</legend><div className="mt-3 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <label className="grid gap-1.5 xl:col-span-4"><span className="text-xs font-bold text-[#766e64]">Importer le fichier numérique {index === 0 ? "*" : ""}</span><input name={`ebook_file_${index}`} type="file" accept=".epub,.pdf,application/epub+zip,application/pdf" required={index === 0} className="rounded-xl border border-[#d9cebd] bg-[#fffaf2] px-3 py-2 text-sm" /><span className="text-[.65rem] text-[#92887c]">PDF ou EPUB : la première page devient automatiquement la couverture. Convertissez les fichiers MOBI en PDF ou EPUB avant l’import.</span></label>
